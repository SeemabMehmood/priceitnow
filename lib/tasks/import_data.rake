...
namespace :import do
  ...
  task data: :environment do
    ...
    bags_info = Roo::Spreadsheet.open('./collector_fin_30072020.xlsx')
    ...
    bags_info.each_with_index do |row, idx|
      ...
      currency            = bags_data['price'].to_s.include?('$') ? '$' : '€'
      ...
      if (Handbag.exists?(reference_no: reference_no) && reference_no.present?) || (Handbag.exists?(name: bags_data['name']) && reference_no.blank?)
        puts "Handbag already exists with name #{bags_data['name']}"
        next
      end

      handbag = Handbag.new(bags_data.except('price'))
      handbag.prices << Price.create(price: bags_data['price'], currency: currency)
